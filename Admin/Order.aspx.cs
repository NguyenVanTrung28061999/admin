...
public partial class ContactList : System.Web.UI.Page
{
    ...
    public void LoadData()
    {
        ...
        var query = db.Orders.OrderByDescending(x => x.CreateTime).AsQueryable();
        ...
        if (catID > 0)
        {
            ...
            query = query.Where(x => x.OrderID == catID);
        }
        ...
        if (totalItems > 0)
        {
            ...
            string url = "/Order.aspx?cat={0}&keyword={1}&page={2}".StringFormat(catID, keyword, "{0}");
    ...
    {
        DBEntities db = new DBEntities();
        var data = db.Orders.OrderBy(x => x.OrderID).ToList();

        DropDownList_Category.DataTextField = "CreateTime";
        DropDownList_Category.DataValueField = "OrderID";
        ...
        ListItem item = new ListItem(".: Chọn thời gian khách Order :.", string.Empty);
    ...
    private void SearchData()
    {
        ...
        string url = "/Order.aspx?cat={0}&keyword={1}".StringFormat(catID, keyword);
